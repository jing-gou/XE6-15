...
SQLite 每个连接默认 `PRAGMA foreign_keys = OFF`。本设计的 `recurring_instances` / `triggers` / `undo_records` 上的 `ON DELETE CASCADE` 与 `reminders` 的拆列外键都依赖外键约束，不开则级联删除不生效、引用完整性无保障，会产生孤儿 reminder 被 trigger 继续扫描到。**因此每个连接打开后必须立即执行**：

```sql
PRAGMA foreign_keys = ON;
PRAGMA journal_mode = WAL;   -- 单写多读，适配 trigger.Run 后台扫描与对话读写并发
```

App 用连接池时须在每个新连接上设置上述 PRAGMA（在 driver 的 connect hook 中执行），而非仅在 DB 打开时设置一次。
...
// CalendarEventEntity：单次日程（event）或点状提醒（point）。
// 可空列统一用 sql.NullString，避免 Scan NULL 报错；领域层再决定零值语义。
type CalendarEventEntity struct {
    ...
    EndTime         sql.NullString `db:"end_time"`         // TEXT，event 必填，point 为 NULL
    Location        sql.NullString `db:"location"`         // TEXT，可空
    Notes           sql.NullString `db:"notes"`            // TEXT，可空
...
type RecurringSeriesEntity struct {
    ...
    PausedUntil     sql.NullString `db:"paused_until"`       // TEXT，仅 paused 时非空
...
type RecurringInstanceEntity struct {
    ...
    EndTime   sql.NullString `db:"end_time"`    // TEXT，event 类有值
    Status    string         `db:"status"`      // 'active' | 'skipped' | 'modified'
    Override  sql.NullString `db:"override"`    // TEXT(JSON)，modified 时覆盖 series 字段
    CreatedAt string         `db:"created_at"`
}
```
...
type ReminderEntity struct {
    ID          string `db:"id"`
    // 多态引用拆为两列独立外键，受 SQLite 外键约束；source_id/source_type 保留为派生查询字段。
    ScheduleID sql.NullString `db:"schedule_id"` // FK -> calendar_events.id，source_type='schedule' 时非空
    InstanceID sql.NullString `db:"instance_id"` // FK -> recurring_instances.id，source_type='instance' 时非空
    SourceID   string         `db:"source_id"`    // = schedule_id 或 instance_id（派生列，便于统一查询）
    SourceType string         `db:"source_type"`  // 'schedule' | 'instance'，与上两列对齐
    Title       string         `db:"title"`
...
CREATE TABLE reminders (
    id           TEXT PRIMARY KEY,
    schedule_id  TEXT REFERENCES calendar_events(id) ON DELETE CASCADE,
    instance_id  TEXT REFERENCES recurring_instances(id) ON DELETE CASCADE,
    -- 多态引用约束：两列恰一非空，且与 source_type 对齐
    source_type  TEXT NOT NULL CHECK(source_type IN ('schedule','instance')),
    source_id    TEXT NOT NULL,  -- = schedule_id 或 instance_id（派生）
    CHECK ( (source_type='schedule'  AND schedule_id IS NOT NULL AND instance_id IS NULL)
         OR (source_type='instance'  AND instance_id IS NOT NULL AND schedule_id IS NULL) ),
    title        TEXT NOT NULL,
...
CREATE INDEX idx_rem_schedule ON reminders(schedule_id);
CREATE INDEX idx_rem_instance ON reminders(instance_id);
...
-- 到期扫描条件含 status='scheduled'，复合索引让查询直接定位待调度记录。
CREATE INDEX idx_trig_due ON triggers(status, fire_time);
...
推迟时长优先级固定为：本次明确时长 > 用户设置值 > 初始默认 10 分钟（不再用 0 混合「未设」与无效值，也不追问）。

```go
type UserPreferenceEntity struct {
    ID                          int    `db:"id"`                          // 固定 = 1
    DefaultSnoozeMinutes        int    `db:"default_snooze_minutes"`      // 默认 10，范围 1..1440
...
CREATE TABLE user_preferences (
    ...
    default_snooze_minutes       INTEGER NOT NULL DEFAULT 10
                                  CHECK(default_snooze_minutes BETWEEN 1 AND 1440),
...
所有操作（创建 / 提醒处理 / 变更 / 撤销 / 日历查询 / 备忘录记录与查询）的结果统一落回执表。`before` / `after` 存 JSON 快照，`scope` 记影响范围，`undo_deadline` 标撤销截止时间。
新增 `conversation_id` / `request_id` 与结构化 `payload`：`conversation_id` 串联回执链，`request_id` 作幂等键（同一请求重试不产生重复回执）。`raw_utterance` 与 `target` 系列（系统触发的 reminder、无单一目标的查询/备忘录回执）相应改为可空。

```go
type ReceiptEntity struct {
    ID             string         `db:"id"`
    Type           string         `db:"type"`           // 'create'|'reminder'|'change'|'undo'|'query'|'note_create'|'note_query'
    ConversationID sql.NullString `db:"conversation_id"` // 会话关联，串联回执链
    RequestID      sql.NullString `db:"request_id"`      // 幂等键，同请求重试跳过
    RawUtterance   sql.NullString `db:"raw_utterance"`    // 用户原话，系统触发/查询可为空
    TargetID       sql.NullString `db:"target_id"`        // 操作对象 ID，无单一目标时为空
    TargetType     sql.NullString `db:"target_type"`      // 'schedule'|'reminder'|'instance'|'series'，可空
    Before         sql.NullString `db:"before"`          // TEXT(JSON)，变更前快照
    After          sql.NullString `db:"after"`            // TEXT(JSON)，变更后快照
    Scope          sql.NullString `db:"scope"`            // '仅本次'|'本次及以后'|'整个系列'，change 有
    Result         string         `db:"result"`           // 操作结果描述
    Payload        sql.NullString `db:"payload"`         // TEXT(JSON)，结构化结果（查询分页、备忘录内容等）
    UndoDeadline   sql.NullString `db:"undo_deadline"`   // TEXT，仅 change 类型
...
CREATE TABLE receipts (
    id              TEXT PRIMARY KEY,
    type            TEXT NOT NULL CHECK(type IN ('create','reminder','change','undo','query','note_create','note_query')),
    conversation_id TEXT,
    request_id      TEXT UNIQUE,             -- 幂等键：同 request_id 重试跳过
    raw_utterance   TEXT,
    target_id       TEXT,
    target_type     TEXT CHECK(target_type IS NULL OR target_type IN ('schedule','reminder','instance','series')),
    before          TEXT,
    after           TEXT,
    scope           TEXT,
...
CREATE INDEX idx_rcpt_conv ON receipts(conversation_id);
```
...
-- 撤销过期清理按 created_at 判断，需要 created_at 索引支持该清理查询。
CREATE INDEX idx_undo_created ON undo_records(created_at);
```
...
type RecurringSeriesRepo interface {
    ...
    Update(s *RecurringSeriesEntity) error      // 含 status / paused_until 变更；终止用 status='terminated'
    Delete(id string) error                     // 物理删除废弃系列及其实例；暂停/终止不调用，保留历史
}
...
type ReceiptRepo interface {
    // 插入回执；若 r.RequestID 已存在（幂等键冲突）则跳过，不报错，返回已有回执 ID。
...
// TxRunner 统一跨 Repository 的 SQLite 事务边界。change 等需要原子写多张表
// （schedule / instance / reminder / trigger / receipt / undo_record）的流程，
// 必须在 WithTx 内执行：任一步失败整体回滚，避免出现部分更新或撤销快照与实际状态不一致。
type TxRunner interface {
    WithTx(ctx context.Context, fn func(ctx context.Context) error) error
}
```

各 Repository 在事务内被绑到同一个 `*sql.Tx`：调用 `WithTx` 时把 `ctx` 注入 Repositories（如经 key 关联 `*sql.Tx`），后续 Repository 调用透明复用该事务；`WithTx` 返回时提交或回滚。领域层组合多 Repository 完成业务时，**所有写多表的流程必须包在 WithTx 内**。
```
...
| change | CalendarEventRepo / RecurringSeriesRepo / RecurringInstanceRepo / ReminderRepo / TriggerRepo / ReceiptRepo / UndoRecordRepo + **TxRunner.WithTx** | 修改字段、跳过实例、暂停/终止系列、快照存撤销，全程同事务 |
...
| 查询场景 | 命中索引 | 说明 |
| --- | --- | --- |
| 到期触发扫描 | `idx_trig_due` | trigger.Run 按 `status='scheduled' AND fire_time <= now` 定位待调度记录，复合索引直达 |
...
| 回执会话串联 | `idx_rcpt_conv` | 按 conversation_id 串联回执链 |
| 便签过期清理 | `idx_note_expires` | 按 expires_at 范围删除过期行 |
| 撤销过期清理 | `idx_undo_created` | 按 created_at 删除超 10 分钟窗口记录 |
